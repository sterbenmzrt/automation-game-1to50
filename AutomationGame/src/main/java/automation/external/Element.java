...
public class Element implements ElementMapper {
    ...
    @Override
    public String getAttributeFrom(By by, String attr) {
        String message = Thread.currentThread().getStackTrace()[1].getMethodName() + ": " + by.toString();
        System.out.println(message);
        String value = webDriver.findElement(by).getAttribute(attr);
    ...
    public String getInnerText(By by){
        String message = Thread.currentThread().getStackTrace()[1].getMethodName() + ": " + by.toString();
        System.out.println(message);
        return (String) ((JavascriptExecutor) webDriver).executeScript("return arguments[0].innerText", webDriver.findElement(by));
    }
